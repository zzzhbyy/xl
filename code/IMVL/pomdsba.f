...
C-----------------------------------------------------------------------
C
       netcdf_file='pomdsba.nc'  ! netCDF output file
...
C-----------------------------------------------------------------------
C
      days=60.e0       ! run duration in days
C
C-----------------------------------------------------------------------
C
      prtd1=1.e0/24.e0     ! Initial print interval (days)
C
C-----------------------------------------------------------------------
C
      prtd2=1.e0/24.e0         ! Final print interval (days)
C
C-----------------------------------------------------------------------
C
      swtch=20.e0      ! Time to switch from prtd1 to prtd2 
C
C-----------------------------------------------------------------------
C
...
C
C****************************************************************byle
              call proftle(uf,wtsurf,tsurf,nbct,tps)
              call proftle(vf,wssurf,ssurf,nbcs,tps)

C****************************************************************endle
C              call proft(uf,wtsurf,tsurf,nbct,tps)
C              call proft(vf,wssurf,ssurf,nbcs,tps)
...
c ---------------------------------------------------------------------
C
      subroutine proftle(f,wfsurf,fsurf,nbc,dh)
C **********************************************************************
C *                                                                    *
...
C********************************************************************byle
      real  p0(im,jm,kb),p1(im,jm,kb),p2(im,jm,kb)
C       real  a(im,jm,kb),b(im,jm,kb),c(im,jm,kb),d(im,jm,kb)
      real  lea(im,jm,kb),leb(im,jm,kb),lec(im,jm,kb),dle(im,jm,kb)
      real  e1,e2,er,ed,dijk,ckbm1,t0le,t1le,t2le
...
C            e1=0
C            e2=0
            er=e1*e1-e1*e2+e2*e2
C            ed=e1-e2
           ed=0
         p0(i,j,k)=(er+2*dzz(k-1)*ed)/(12.e0*dzz(k)*dz(k))
         p2(i,j,k)=(er-2*dzz(k)*ed)/(12.e0*dzz(k-1)*dz(k))
         p1(i,j,k)=-(er+2*(dzz(k-1)-dzz(k))*ed)/
     $           (6.e0*dzz(k-1)*dzz(k))+1.e0
C             ed=-1/8*dzz(k)
             t0le=2.e0*(3.e0*(kh(i,j,k)+umol)*(dzz(k)-dzz(k-1))
     $         +6.e0*dzz(k-1)*(kh(i,j,k+1)+umol) 
     $         +4.e0*(kh(i,j,k+1)-kh(i,j,k))*ed)
     $         /(12.e0*dz(k)*dz(k)*dzz(k))


            t1le=2.e0*(3.e0*dzz(k)*(kh(i,j,k+1)-2*kh(i,j,k)-umol)
     $              + 3.e0*dzz(k-1)*(kh(i,j,k)-2*kh(i,j,k+1)-umol)
     $              -4.e0*(kh(i,j,k+1)-kh(i,j,k))*ed)
     $               /(6.e0*dzz(k)*dzz(k-1)*dz(k))

            t2le=2.e0*(3.e0*(kh(i,j,k+1)+umol)*(dzz(k-1)-dzz(k))
     $         +6.e0*dzz(k)*(kh(i,j,k)+umol) 
     $         +4.e0*(kh(i,j,k+1)-kh(i,j,k))*ed)
     $         /(12.e0*dz(k)*dz(k)*dzz(k-1))
 

C         p0(i,j,k)=0
C         p2(i,j,k)=0
C         p1(i,j,k)=1.e0
C             t0le=(kh(i,j,k+1)+umol)
C     $         /(dz(k)*dzz(k))


C            t1le=-((kh(i,j,k)+umol)/dzz(k-1)
C     $             +(kh(i,j,k+1)+umol)/dzz(k))/dz(k)    

C            t2le=(kh(i,j,k)+umol)/(dz(k)*dzz(k-1))
          
            


            lea(i,j,k)=dti2*t2le/(dh(i,j)*dh(i,j))-p2(i,j,k)
            leb(i,j,k)=dti2*t1le/(dh(i,j)*dh(i,j))-p1(i,j,k)
            lec(i,j,k)=dti2*t0le/(dh(i,j)*dh(i,j))-p0(i,j,k)
...
C        do j=1,jm
C          do i=1,im
C             a(i,j,k-1)=-dti2*(kh(i,j,k)+umol)
c     $                  /(dz(k-1)*dzz(k-1)*dh(i,j)*dh(i,j))
C            c(i,j,k)=-dti2*(kh(i,j,k)+umol)
C     $                  /(dz(k)*dzz(k-1)*dh(i,j)*dh(i,j))
C          end do
C        end do
C      end do
...
C
        do j=1,jm
          do i=1,im
C*******************************************************************byle
            a(i,j,1)=-dti2*(kh(i,j,2)+umol)
     $                  /(dz(1)*dzz(1)*dh(i,j)*dh(i,j))
C******************************************************************endle
...
C******************************************************************byle
      do k=1,kbm1
        do j=1,jm
          do i=1,im
            dle(i,j,k)=-f(i,j,k)+dti2*(rad(i,j,k)-rad(i,j,k+1))
     $                   /(dh(i,j)*dz(k))
...
C
      do k=2,kbm2
        do j=1,jm
          do i=1,im
            dijk=p0(i,j,k)*dle(i,j,k+1)
     $           +p1(i,j,k)*dle(i,j,k)+p2(i,j,k)*dle(i,j,k-1)

            gg(i,j,k)=1.e0/(lea(i,j,k)*ee(i,j,k-1)+leb(i,j,k))
            ee(i,j,k)=-lec(i,j,k)*gg(i,j,k)
            gg(i,j,k)=(dijk-lea(i,j,k)*gg(i,j,k-1))
     $                 *gg(i,j,k)
          end do
        end do
      end do
...
C        do j=1,jm
C          do i=1,im
C            gg(i,j,k)=1.e0/(a(i,j,k)+c(i,j,k)*(1.e0-ee(i,j,k-1))-1.e0)
C            ee(i,j,k)=a(i,j,k)*gg(i,j,k)
C            gg(i,j,k)=(c(i,j,k)*gg(i,j,k-1)-f(i,j,k)
C     $                 +dti2*(rad(i,j,k)-rad(i,j,k+1))
C     $                   /(dh(i,j)*dz(k)))
C     $                 *gg(i,j,k)
C          end do
C        end do
...
C****************************************************************byle
      do j=1,jm
        do i=1,im
          ckbm1=-dti2*(kh(i,j,kbm1)+umol)
     $                /(dz(kbm1)*dzz(kbm1-1)*dh(i,j)*dh(i,j))
          
          f(i,j,kbm1)=(ckbm1*gg(i,j,kbm2)-f(i,j,kbm1))
     $               /(ckbm1*(1.e0-ee(i,j,kbm2))-1.e0)
        end do
      end do
C
C****************************************************************endle
C      do j=1,jm
C        do i=1,im
C          f(i,j,kbm1)=(c(i,j,kbm1)*gg(i,j,kbm2)-f(i,j,kbm1)
C     $                 +dti2*(rad(i,j,kbm1)-rad(i,j,kb))
C     $                   /(dh(i,j)*dz(kbm1)))
C     $                 /(c(i,j,kbm1)*(1.e0-ee(i,j,kbm2))-1.e0)
C        end do
C      end do
C
...
C     Define depth:
C
      xm_le=(east_c(1,1)+east_c(im,1))*0.5e0
      k_le=40/(east_c(im,1)-east_c(1,1))
 
      do i=1,im
        do j=1,jm
C
C           h(i,j)=4500.e0*(1.e0-delh*exp(-((east_c(i,j)
C          h(i,j)=0.1e0*4500.e0*(1.e0-delh*exp(-((east_c(i,j)
C     $                                   -east_c((im+1)/2,j))**2
C     $                                +(north_c(i,j)
C     $                                   -north_c(i,(jm+1)/2))**2)
C     $                                /ra**2))

          h(i,j)=4500.e0*(1.e0-0.6e0/cosh(k_le*(east_c(i,j)-xm_le)))
...
C
      do k=1,kbm1
        do j=1,jm
          do i=1,im
C            tb(i,j,k)=5.e0+15.e0*exp(zz(k)*h(i,j)/1000.e0)-tbias
C            sb(i,j,k)=35.e0-sbias
            tb(i,j,k)=5.e0+25.e0*exp(zz(k)*h(i,j)/1000.e0)-tbias
            sb(i,j,k)=35.5e0-1.e0*exp(zz(k)*h(i,j)/1000.e0)
            tclim(i,j,k)=tb(i,j,k)
            sclim(i,j,k)=sb(i,j,k)
